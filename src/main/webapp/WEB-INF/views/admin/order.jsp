        ...
        <div class="container-fluid">
            ...
            <div class="card-header d-flex align-items-center justify-content-between">
                ...
                <div class="dropdown text-end">
                    <button class="btn btn-outline-dark dropdown-toggle text-white" type="button" id="statusFilterDropdown" data-bs-toggle="dropdown">
                        Trạng Thái
                    </button>
                    <ul class="dropdown-menu dropdown-menu-end">
                        <li><a class="dropdown-item" href="#" onclick="filterOrders('')">Tất cả</a></li>
                        <li><a class="dropdown-item" href="#" onclick="filterOrders('DaXacNhan')">Đã xác nhận</a></li>
                        <li><a class="dropdown-item" href="#" onclick="filterOrders('ChoXacNhan')">Chờ xác nhận</a></li>
                        <li><a class="dropdown-item" href="#" onclick="filterOrders('HoanThanh')">Hoàn thành</a></li>
                        <li><a class="dropdown-item" href="#" onclick="filterOrders('DangVanChuyen')">Đang vận chuyển</a></li>
                        <li><a class="dropdown-item" href="#" onclick="filterOrders('DaHuy')">Đã hủy</a></li>
                    </ul>
                </div>
                <div class="dropdown text-end">
                    <button class="btn btn-outline-dark dropdown-toggle text-white" type="button" id="methodFilterDropdown" data-bs-toggle="dropdown">
                        PT Thanh Toán
                    </button>
                    <ul class="dropdown-menu dropdown-menu-end">
                        <li><a class="dropdown-item" href="#" onclick="filterPayment('')">Tất cả</a></li>
                        <li><a class="dropdown-item" href="#" onclick="filterPayment('ChuyenKhoan')">ChuyenKhoan</a></li>
                        <li><a class="dropdown-item" href="#" onclick="filterPayment('COD')">COD</a></li>
                    </ul>
                </div>
            </div>
            <div class="alert-container m-1">
                <c:if test="${param.msg != null}">
                    <div class="alert alert-success" id="successMessage">
                        ...
                        <c:choose>
                            <c:when test="${param.msg == 'created'}">Tạo đơn hàng thành công!</c:when>
                            <c:when test="${param.msg == 'updated'}">Cập nhật đơn hàng thành công!</c:when>
                            <c:when test="${param.msg == 'deleted'}">Xóa đơn hàng thành công!</c:when>
                        </c:choose>
                    </div>
                </c:if>
                        ...
                        <tbody id="orderTableBody">
                            ...
                            <c:forEach var="o" items="${orders}">
                                <tr data-status="${o.status}" data-method="${o.payment.method}">
                                    ...
                                    <td>
                                        <c:choose>
                                            <c:when test="${o.status eq 'DaXacNhan'}"><span class="badge bg-primary">Đã Xác Nhận</span></c:when>
                                            <c:when test="${o.status eq 'ChoXacNhan'}"><span class="badge bg-warning text-dark">Chờ Xác Nhận</span></c:when>
                                            <c:when test="${o.status eq 'HoanThanh'}"><span class="badge bg-success">Hoàn Thành</span></c:when>
                                            <c:when test="${o.status eq 'DaHuy'}"><span class="badge bg-danger">Đã Hủy</span></c:when>
                                            <c:when test="${o.status eq 'DangVanChuyen'}"><span class="badge bg-secondary">Đang Vận Chuyển</span></c:when>
                                            <c:otherwise><span class="badge bg-secondary">${o.status}</span></c:otherwise>
                                        </c:choose>
                                    </td>
                                    <td>${o.payment.method}</td>
                                    <td>
                                        <a href="${pageContext.request.contextPath}/admin/order?action=detail&id=${o.orderId}" class="btn btn-info btn-sm"><i class="fas fa-eye"></i></a>
                                        <a class="btn btn-warning btn-sm edit-service-btn" data-id="${o.orderId}" data-status="${o.status}">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a href="${pageContext.request.contextPath}/admin/order?action=delete&id=${o.orderId}&userId=${o.user.userId}" class="btn btn-danger btn-sm" onclick="return confirm('Xác nhận xoá đơn hàng?')">
            ...
            <div id="editServiceModal" class="modal-overlay" style="display: none;">
                <div class="modal-content">
                    ...
                    <form action="${pageContext.request.contextPath}/admin/order" method="post">
                        ...
                        <label for="statusSelectService">Đổi trạng thái đơn hàng:</label>
